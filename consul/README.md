...
[![s390x/consul build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/s390x/job/consul.svg?label=s390x/consul%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/s390x/job/consul/)
...
Running the Consul container with no arguments will give you a Consul server in [development mode](https://www.consul.io/docs/agent/options.html#_dev). The provided entry point script will also look for Consul subcommands and run `consul` as the correct user and with that subcommand. For example, you can execute `docker run s390x/consul members` and it will run the `consul members` command inside the container. The entry point also adds some special configuration options as detailed in the sections below when running the `agent` subcommand. Any other command gets `exec`-ed inside the container under `dumb-init`.
...
```console
$ docker run -d --name=dev-consul -e CONSUL_BIND_INTERFACE=eth0 s390x/consul
```
...
```console
$ docker run -d -e CONSUL_BIND_INTERFACE=eth0 s390x/consul agent -dev -join=172.17.0.2
... server 2 starts
$ docker run -d -e CONSUL_BIND_INTERFACE=eth0 s390x/consul agent -dev -join=172.17.0.2
... server 3 starts
```
...
```console
$ docker exec -t dev-consul s390x/consul members
...
```console
$  docker run -d --net=host -e 'CONSUL_LOCAL_CONFIG={"leave_on_terminate": true}' s390x/consul agent -bind=<external ip> -retry-join=<root agent ip>
...
```console
docker run -d --net=host s390x/consul agent -bind=<external ip> -client=<bridge ip> -retry-join=<root agent ip>
==> Starting Consul agent...
==> Starting Consul agent RPC...
==> Consul agent running!
         Node name: 'linode'
...
```console
$ docker run -d --net=host -e 'CONSUL_LOCAL_CONFIG={"skip_leave_on_interrupt": true}' s390x/consul agent -server -bind=<external ip> -retry-join=<root agent ip> -bootstrap-expect=<number of server agents>
```
...
```console
$ docker run -d --net=host -e 'CONSUL_ALLOW_PRIVILEGED_PORTS=' s390x/consul -dns-port=53 -recursor=8.8.8.8
```
...
```console
$ docker run -d --net=host -e 'CONSUL_ALLOW_PRIVILEGED_PORTS=' s390x/consul agent -dns-port=53 -recursor=8.8.8.8 -bind=<bridge ip>
```
