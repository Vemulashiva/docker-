...
[![arm32v6/adoptopenjdk build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/adoptopenjdk.svg?label=arm32v6/adoptopenjdk%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/adoptopenjdk/)
...
```dockerfile
FROM arm32v6/adoptopenjdk:11-jre-hotspot
RUN mkdir /opt/app
...
```dockerfile
FROM arm32v6/adoptopenjdk:11-jre-openj9
RUN mkdir /opt/app
COPY japp.jar /opt/app
CMD ["java", "-jar", "/opt/app/japp.jar"]
```
...
```dockerfile
FROM arm32v6/adoptopenjdk:12.0.1_12-jdk-openj9-0.14.1
CMD ["java", "-jar", "/opt/app/japp.jar"]
```
