...
[![i386/spiped build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/i386/job/spiped.svg?label=i386/spiped%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/i386/job/spiped/)
...
```console
$ docker run -d -v /path/to/keyfile:/spiped/key:ro -p 8025:8025 --init i386/spiped -d -s '[0.0.0.0]:8025' -t '[127.0.0.1]:25'
```
...
```console
$ docker run -d -v /path/to/keyfile:/spiped/key:ro -p 9200:9200 --link elasticsearch:elasticsearch --init i386/spiped -d -s '[0.0.0.0]:9200' -t 'elasticsearch:9200'
```

If you don’t need any to bind to a privileged port you can pass `--user spiped` to make *spiped* run as an unprivileged user:

```console
$ docker run -d -v /path/to/keyfile:/spiped/key:ro --user spiped -p 9200:9200 --link elasticsearch:elasticsearch --init i386/spiped -d -s '[0.0.0.0]:9200' -t 'elasticsearch:9200'
```
...
The `i386/spiped` images come in many flavors, each designed for a specific use case.

## `i386/spiped:<version>`

This is the defacto image. If you are unsure about what your needs are, you probably want to use this one. It is designed to be used both as a throw away container (mount your source code and start the container to start your app), as well as the base to build other images off of.

## `i386/spiped:<version>-alpine`
