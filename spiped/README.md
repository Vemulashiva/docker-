...
[![ppc64le/spiped build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/spiped.svg?label=ppc64le/spiped%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/spiped/)
...
```console
$ docker run -it --rm ppc64le/spiped
...
```console
$ docker run -d -v /path/to/keyfile:/spiped/key:ro -p 8025:8025 --init ppc64le/spiped -d -s '[0.0.0.0]:8025' -t '[127.0.0.1]:25'
```
...
```console
$ docker run -d -v /path/to/keyfile:/spiped/key:ro -p 9200:9200 --link elasticsearch:elasticsearch --init ppc64le/spiped -d -s '[0.0.0.0]:9200' -t 'elasticsearch:9200'
```

If you don’t need any to bind to a privileged port you can pass `--user spiped` to make *spiped* run as an unprivileged user:

```console
$ docker run -d -v /path/to/keyfile:/spiped/key:ro --user spiped -p 9200:9200 --link elasticsearch:elasticsearch --init ppc64le/spiped -d -s '[0.0.0.0]:9200' -t 'elasticsearch:9200'
```
...
The `ppc64le/spiped` images come in many flavors, each designed for a specific use case.
