...
-	[`centos7`, `7`](https://github.com/CentOS/sig-cloud-instance-images/blob/8ea12f93f7e4e0a524ce8bf2af4da8fa5a90aa86/docker/Dockerfile)
-	[`centos7.6.1810`, `7.6.1810`](https://github.com/CentOS/sig-cloud-instance-images/blob/c3f4c6a2687b6cd88ea39dcd4bcb751fe6482442/docker/Dockerfile)

[![ppc64le/centos build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/centos.svg?label=ppc64le/centos%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/centos/)
...
The `ppc64le/centos:latest` tag is always the most recent version currently available.

## Rolling builds

The CentOS Project offers regularly updated images for all active releases. These images will be updated monthly or as needed for emergency fixes. These rolling updates are tagged with the major version number only. For example: `docker pull ppc64le/centos:6` or `docker pull ppc64le/centos:7`
...
For example: `docker pull ppc64le/centos:5.11` or `docker pull ppc64le/centos:6.6`

## Overlayfs and yum

Recent Docker versions support the [overlayfs](https://docs.docker.com/engine/userguide/storagedriver/overlayfs-driver/) backend, which is enabled by default on most distros supporting it from Docker 1.13 onwards. On Centos 6 and 7, **that backend requires yum-plugin-ovl to be installed and enabled**; while it is installed by default in recent ppc64le/centos images, make it sure you retain the `plugins=1` option in `/etc/yum.conf` if you update that file; otherwise, you may encounter errors related to rpmdb checksum failure - see [Docker ticket 10180](https://github.com/docker/docker/issues/10180) for more details.
...
Systemd is now included in both the ppc64le/centos:7 and ppc64le/centos:latest base containers, but it is not active by default. In order to use systemd, you will need to include text similar to the example Dockerfile below:
