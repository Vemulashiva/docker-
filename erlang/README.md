...
[![arm32v6/erlang build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/erlang.svg?label=arm32v6/erlang%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/erlang/)
...
```console
➸ docker run -it --rm arm32v6/erlang
...
User switch command
 --> ?
  ...
  ? | h             - this message
 --> q
➸ docker run -it --rm -h erlang.local arm32v6/erlang erl -name snode@erlang.local
Erlang/OTP 20 [erts-9.0] [source] [64-bit] [smp:8:8] [ds:8:8:10] [async-threads:10] [hipe] [kernel-poll:false]
...
```console
$ docker run -it --rm --name erlang-inst1 -v "$PWD":/usr/src/myapp -w /usr/src/myapp arm32v6/erlang escript your-escript.erl
```
