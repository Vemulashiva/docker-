...
[![i386/jetty build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/i386/job/jetty.svg?label=i386/jetty%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/i386/job/jetty/)
...
```console
$ docker run -d -p 80:8080 -p 443:8443 i386/jetty
```
...
```console
$ docker run -e JAVA_OPTIONS="-Xmx1g" -d i386/jetty
```

## Read-only container

To run `i386/jetty` as a read-only container, have Docker create the `/tmp/jetty` and `/run/jetty` directories as volumes:

```console
$ docker run -d --read-only -v /tmp/jetty -v /run/jetty i386/jetty
```
...
```console
$ docker run -d -u jetty i386/jetty
```

# Image Variants

The `i386/jetty` images come in many flavors, each designed for a specific use case.

## `i386/jetty:<version>`

This is the defacto image. If you are unsure about what your needs are, you probably want to use this one. It is designed to be used both as a throw away container (mount your source code and start the container to start your app), as well as the base to build other images off of.

## `i386/jetty:<version>-alpine`
