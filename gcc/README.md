...
[![winamd64/gcc build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/gcc.svg?label=winamd64/gcc%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/gcc/)
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp winamd64/gcc:4.9 gcc -o myapp myapp.c
```
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp winamd64/gcc:4.9 make
```
