...
[![winamd64/backdrop build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/backdrop.svg?label=winamd64/backdrop%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/backdrop/)
...
```console
$ docker run --name some-backdrop --link some-mysql:mysql -d winamd64/backdrop
```
...
```console
$ docker run --name some-backdrop --link some-mysql:mysql -p 8080:80 -d winamd64/backdrop
```
