...
[![winamd64/gazebo build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/gazebo.svg?label=winamd64/gazebo%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/gazebo/)
...
```dockerfile
FROM winamd64/gazebo:gzserver8
...
```console
$ docker run -v "/home/ubuntu/.gazebo/:/root/.gazebo/" winamd64/gazebo
```
...
```console
$ docker run -d -v="/tmp/.gazebo/:/root/.gazebo/" --name=gazebo winamd64/gazebo
```
...
```console
$ docker exec -it winamd64/gazebo bash
