...
[![winamd64/ghost build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/ghost.svg?label=winamd64/ghost%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/ghost/)
...
```console
$ docker run -d --name some-ghost winamd64/ghost
```
...
```console
$ docker run -d --name some-ghost -e url=http://localhost:3001 -p 3001:2368 winamd64/ghost
```
...
```console
$ docker run -d --name some-ghost -p 3001:2368 -v /path/to/ghost/blog:/var/lib/ghost/content winamd64/ghost:1-alpine
```

### Ghost 0.11.xx

```console
$ docker run -d --name some-ghost -p 3001:2368 -v /path/to/ghost/blog:/var/lib/ghost winamd64/ghost:0.11-alpine
```
...
```console
$ docker run -d --name some-ghost --volumes-from some-ghost-data winamd64/ghost
```
...
```console
$ docker run -d --name some-ghost -e url=http://some-ghost.example.com winamd64/ghost
```
