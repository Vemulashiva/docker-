...
[![i386/swift build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/i386/job/swift.svg?label=i386/swift%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/i386/job/swift/)
...
```bash
docker run --cap-add sys_ptrace -it --rm i386/swift swift
```

##### Pull the Docker Image From Docker Hub:

```bash
docker pull i386/swift
```

##### Create a Container from the Image and Attach It:

```bash
docker run  -it --name swiftfun i386/swift /bin/bash
```
