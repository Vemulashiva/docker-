...
[![winamd64/elixir build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/elixir.svg?label=winamd64/elixir%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/elixir/)
...
```console
➸ docker run -it --rm winamd64/elixir
...
"1.2.1"
iex(2)>
➸ docker run -it --rm -h elixir.local winamd64/elixir iex --sname snode
Erlang/OTP 18 [erts-7.2.1] [source] [64-bit] [smp:8:8] [async-threads:10] [hipe] [kernel-poll:false]
...
```console
$ docker run -it --rm --name elixir-inst1 -v "$PWD":/usr/src/myapp -w /usr/src/myapp winamd64/elixir elixir your-escript.exs
```
