...
[![s390x/rust build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/s390x/job/rust.svg?label=s390x/rust%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/s390x/job/rust/)
...
```console
$ docker run --rm --user "$(id -u)":"$(id -g)" -v "$PWD":/usr/src/myapp -w /usr/src/myapp s390x/rust:1.23.0 cargo build --release
```
