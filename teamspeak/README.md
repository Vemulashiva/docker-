...
[![winamd64/teamspeak build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/teamspeak.svg?label=winamd64/teamspeak%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/teamspeak/)
...
```console
$ docker run -e TS3SERVER_LICENSE=view winamd64/teamspeak
```

To start a TeamSpeak server, accept the license agreement, and map the ports to the host:

```console
$ docker run -p 9987:9987/udp -p 10011:10011 -p 30033:30033 -e TS3SERVER_LICENSE=accept winamd64/teamspeak
```
...
```console
$ docker run --name some-teamspeak -v /location/to/licensekey.dat:/var/ts3server/licensekey.dat winamd64/teamspeak:tag
```
...
```console
$ docker run --name some-teamspeak -v /my/own/datadir:/var/ts3server/ -d winamd64/teamspeak:tag
```
