...
[![arm32v6/teamspeak build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/teamspeak.svg?label=arm32v6/teamspeak%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/teamspeak/)
...
```console
$ docker run -e TS3SERVER_LICENSE=view arm32v6/teamspeak
```

To start a TeamSpeak server, accept the license agreement, and map the ports to the host:

```console
$ docker run -p 9987:9987/udp -p 10011:10011 -p 30033:30033 -e TS3SERVER_LICENSE=accept arm32v6/teamspeak
```
...
```console
$ docker run --name some-teamspeak -v /location/to/licensekey.dat:/var/ts3server/licensekey.dat arm32v6/teamspeak:tag
```
...
```console
$ docker run --name some-teamspeak -v /my/own/datadir:/var/ts3server/ -d arm32v6/teamspeak:tag
```
