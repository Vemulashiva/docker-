...
[![s390x/teamspeak build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/s390x/job/teamspeak.svg?label=s390x/teamspeak%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/s390x/job/teamspeak/)
...
```console
$ docker run -e TS3SERVER_LICENSE=view s390x/teamspeak
```

To start a TeamSpeak server, accept the license agreement, and map the ports to the host:

```console
$ docker run -p 9987:9987/udp -p 10011:10011 -p 30033:30033 -e TS3SERVER_LICENSE=accept s390x/teamspeak
```
...
```console
$ docker run --name some-teamspeak -v /location/to/licensekey.dat:/var/ts3server/licensekey.dat s390x/teamspeak:tag
```
...
```console
$ docker run --name some-teamspeak -v /my/own/datadir:/var/ts3server/ -d s390x/teamspeak:tag
```
