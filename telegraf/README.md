...
[![arm32v5/telegraf build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v5/job/telegraf.svg?label=arm32v5/telegraf%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v5/job/telegraf/)
...
```console
$ docker run --net=container:influxdb arm32v5/telegraf
```
...
```console
$ docker run --rm arm32v5/telegraf telegraf config > telegraf.conf
```

Once you've customized `telegraf.conf`, you can run the Telegraf container with it mounted in the expected location:

```console
$ docker run -v $PWD/telegraf.conf:/etc/telegraf/telegraf.conf:ro arm32v5/telegraf
```
