...
[![i386/telegraf build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/i386/job/telegraf.svg?label=i386/telegraf%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/i386/job/telegraf/)
...
```console
$ docker run --net=container:influxdb i386/telegraf
```
...
```console
$ docker run --rm i386/telegraf telegraf config > telegraf.conf
```

Once you've customized `telegraf.conf`, you can run the Telegraf container with it mounted in the expected location:

```console
$ docker run -v $PWD/telegraf.conf:/etc/telegraf/telegraf.conf:ro i386/telegraf
```
