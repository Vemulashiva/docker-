...
[![winamd64/telegraf build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/telegraf.svg?label=winamd64/telegraf%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/telegraf/)
...
```console
$ docker run --net=container:influxdb winamd64/telegraf
```
...
```console
$ docker run --rm winamd64/telegraf telegraf config > telegraf.conf
```

Once you've customized `telegraf.conf`, you can run the Telegraf container with it mounted in the expected location:

```console
$ docker run -v $PWD/telegraf.conf:/etc/telegraf/telegraf.conf:ro winamd64/telegraf
```
