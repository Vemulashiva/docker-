...
[![winamd64/arangodb build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/arangodb.svg?label=winamd64/arangodb%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/windows-amd64/job/arangodb/)
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 -d --name arangodb-instance winamd64/arangodb
```

Will create and launch the winamd64/arangodb docker instance as background process. The Identifier of the process is printed. By default ArangoDB listen on port 8529 for request and the image includes `EXPOSE 8529`. If you link an application container it is automatically available in the linked container. See the following examples.
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 --name my-app --link arangodb-instance:db-link winamd64/arangodb
```
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 -p 8529:8529 -d winamd64/arangodb
```
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 winamd64/arangodb arangod --help
```
...
```console
unix> docker create --name arangodb-persist winamd64/arangodb true
```

And use this data container in your ArangoDB container.

```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 --volumes-from arangodb-persist -p 8529:8529 winamd64/arangodb
```
