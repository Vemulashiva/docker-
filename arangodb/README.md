...
[![arm64v8/arangodb build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm64v8/job/arangodb.svg?label=arm64v8/arangodb%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm64v8/job/arangodb/)
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 -d --name arangodb-instance arm64v8/arangodb
```

Will create and launch the arm64v8/arangodb docker instance as background process. The Identifier of the process is printed. By default ArangoDB listen on port 8529 for request and the image includes `EXPOSE 8529`. If you link an application container it is automatically available in the linked container. See the following examples.
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 --name my-app --link arangodb-instance:db-link arm64v8/arangodb
```
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 -p 8529:8529 -d arm64v8/arangodb
```
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 arm64v8/arangodb arangod --help
```
...
```console
unix> docker create --name arangodb-persist arm64v8/arangodb true
```

And use this data container in your ArangoDB container.

```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 --volumes-from arangodb-persist -p 8529:8529 arm64v8/arangodb
```
