...
[![arm32v5/arangodb build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v5/job/arangodb.svg?label=arm32v5/arangodb%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v5/job/arangodb/)
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 -d --name arangodb-instance arm32v5/arangodb
```

Will create and launch the arm32v5/arangodb docker instance as background process. The Identifier of the process is printed. By default ArangoDB listen on port 8529 for request and the image includes `EXPOSE 8529`. If you link an application container it is automatically available in the linked container. See the following examples.
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 --name my-app --link arangodb-instance:db-link arm32v5/arangodb
```
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 -p 8529:8529 -d arm32v5/arangodb
```
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 arm32v5/arangodb arangod --help
```
...
```console
unix> docker create --name arangodb-persist arm32v5/arangodb true
```

And use this data container in your ArangoDB container.

```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 --volumes-from arangodb-persist -p 8529:8529 arm32v5/arangodb
```
