...
[![s390x/mongo build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/s390x/job/mongo.svg?label=s390x/mongo%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/s390x/job/mongo/)
...
```console
$ docker run -it --network some-network --rm s390x/mongo mongo --host some-mongo test
```
...
```console
$ docker run --name some-mongo -d s390x/mongo --serviceExecutor adaptive
```
