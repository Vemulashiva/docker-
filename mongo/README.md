...
[![arm32v6/mongo build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/mongo.svg?label=arm32v6/mongo%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/mongo/)
...
```console
$ docker run -it --network some-network --rm arm32v6/mongo mongo --host some-mongo test
```
...
```console
$ docker run --name some-mongo -d arm32v6/mongo --serviceExecutor adaptive
```
...
$ docker run -it --rm --network some-network arm32v6/mongo \
