...
[![i386/mongo build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/i386/job/mongo.svg?label=i386/mongo%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/i386/job/mongo/)
...
```console
$ docker run -it --network some-network --rm i386/mongo mongo --host some-mongo test
```
...
```console
$ docker run --name some-mongo -d i386/mongo --serviceExecutor adaptive
```
...
$ docker run -it --rm --network some-network i386/mongo \
