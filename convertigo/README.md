...
[![s390x/convertigo build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/s390x/job/convertigo.svg?label=s390x/convertigo%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/s390x/job/convertigo/)
...
```console
$ docker run -d --name C8O-MBAAS -e JAVA_OPTS="-DjvmRoute=server1" -p 28080:28080 s390x/convertigo
```
...
```console
$ docker run -d --name C8O-MBAAS -e JXMX="4096" -p 28080:28080 s390x/convertigo
```
...
```console
$ docker run -d --name C8O-MBAAS -e COOKIE_PATH="/convertigo" -p 28080:28080 s390x/convertigo
```
