...
```console
$ docker run --rm arm32v5/influxdb influxd config > influxdb.conf
```
...
$ docker run -p 8086:8086 \
      ...
      arm32v5/influxdb -config /etc/influxdb/influxdb.conf
```
...
```console
$ docker run --name=influxdb -d -p 8086:8086 arm32v5/influxdb
```
...
```console
$ docker run --rm --link=influxdb -it arm32v5/influxdb influx -host influxdb
```
