...
[![i386/influxdb build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/i386/job/influxdb.svg?label=i386/influxdb%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/i386/job/influxdb/)
...
```console
$ docker run --rm i386/influxdb influxd config > influxdb.conf
```
...
$ docker run -p 8086:8086 \
      ...
      i386/influxdb -config /etc/influxdb/influxdb.conf
```
...
```console
$ docker run --name=influxdb -d -p 8086:8086 i386/influxdb
```
...
```console
$ docker run --rm --link=influxdb -it i386/influxdb influx -host influxdb
```
