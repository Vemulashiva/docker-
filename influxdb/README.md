...
[![arm32v6/influxdb build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/influxdb.svg?label=arm32v6/influxdb%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/influxdb/)
...
```console
$ docker run --rm arm32v6/influxdb influxd config > influxdb.conf
```
...
$ docker run -p 8086:8086 \
      ...
      arm32v6/influxdb -config /etc/influxdb/influxdb.conf
```
...
```console
$ docker run --name=influxdb -d -p 8086:8086 arm32v6/influxdb
```
...
```console
$ docker run --rm --link=influxdb -it arm32v6/influxdb influx -host influxdb
```
