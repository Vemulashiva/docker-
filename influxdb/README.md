...
[![ppc64le/influxdb build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/influxdb.svg?label=ppc64le/influxdb%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/influxdb/)
...
```console
$ docker run --rm ppc64le/influxdb influxd config > influxdb.conf
```
...
$ docker run -p 8086:8086 \
      ...
      ppc64le/influxdb -config /etc/influxdb/influxdb.conf
```
...
```console
$ docker run --name=influxdb -d -p 8086:8086 ppc64le/influxdb
```
...
```console
$ docker run --rm --link=influxdb -it ppc64le/influxdb influx -host influxdb
```
