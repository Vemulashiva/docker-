...
[![ppc64le/r-base build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/r-base.svg?label=ppc64le/r-base%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/r-base/)
...
```console
$ docker run -ti --rm ppc64le/r-base
```
...
```console
$ docker run -ti --rm -v "$PWD":/home/docker -w /home/docker -u docker ppc64le/r-base R CMD check .
```
...
```console
$ docker run -ti --rm ppc64le/r-base /usr/bin/bash
$ vim.tiny myscript.R
```
