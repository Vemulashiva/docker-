...
-	[`4.7.4-standalone`, `4.7-standalone`, `4-standalone`, `standalone`, `4.7.4`, `4.7`, `4`, `latest`](https://github.com/TimWolla/docker-adminer/blob/d564e3ea7af5d3335527d9b9eab30555967e458f/4/Dockerfile)
-	[`4.7.4-fastcgi`, `4.7-fastcgi`, `4-fastcgi`, `fastcgi`](https://github.com/TimWolla/docker-adminer/blob/b8632301cfc1d850ffcc0672f5d36b005a5752ea/4/fastcgi/Dockerfile)

[![i386/adminer build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/i386/job/adminer.svg?label=i386/adminer%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/i386/job/adminer/)
...
```console
$ docker run --link some_database:db -p 9000:9000 i386/adminer:fastcgi
```
...
```console
$ docker run --link some_database:db -p 8080:8080 -e ADMINER_PLUGINS='tables-filter tinymce' i386/adminer
```

If a plugin *requires* parameters to work correctly you will need to add a custom file to the container:

```console
$ docker run --link some_database:db -p 8080:8080 -e ADMINER_PLUGINS='login-servers' i386/adminer
...
```console
$ docker run --link some_database:db -p 8080:8080 -e ADMINER_DESIGN='nette' i386/adminer
```
...
```console
docker run -p 8080:8080 -e ADMINER_DEFAULT_SERVER=mysql i386/adminer
```
