...
[![arm32v5/rabbitmq build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v5/job/rabbitmq.svg?label=arm32v5/rabbitmq%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v5/job/rabbitmq/)
...
```console
$ docker run -d --hostname some-rabbit --name some-rabbit --network some-network -e RABBITMQ_ERLANG_COOKIE='secret cookie here' arm32v5/rabbitmq:3
```

This can then be used from a separate instance to connect:

```console
$ docker run -it --rm --network some-network -e RABBITMQ_ERLANG_COOKIE='secret cookie here' arm32v5/rabbitmq:3 bash
...
```console
$ docker run -it --rm --network some-network -e RABBITMQ_ERLANG_COOKIE='secret cookie here' -e RABBITMQ_NODENAME=rabbit@some-rabbit arm32v5/rabbitmq:3 bash
